  ...
  describe("smoke tests", () => {
    ...
    equalUnits.forEach(({ a, b }) => {
      it(`${a.toString()} equals ${b.toString()}`, () => {
        expect(a.in(b.unit).amount).toBeCloseTo(b.amount, 0.001);
      });
    });
  });
  ...
  describe("sin/asin", () => {
    ...
    tests.forEach((test) => {
      it(`sin(${test.angle.toString()}) = ${test.sin}`, () => {
        expect(sin(test.angle)).toBeCloseTo(test.sin, 0.0001);
      });
  ...
  describe("cos/acos", () => {
    ...
    tests.forEach((test) => {
      it(`cos(${test.angle.toString()}) = ${test.cos}`, () => {
        expect(cos(test.angle)).toBeCloseTo(test.cos, 0.0001);
      });
  ...
  describe("tan/atan", () => {
    ...
    tests.forEach((test) => {
      it(`tan(${test.angle.toString()}) = ${test.tan}`, () => {
        expect(tan(test.angle)).toBeCloseTo(test.tan, 0.0001);
      });
